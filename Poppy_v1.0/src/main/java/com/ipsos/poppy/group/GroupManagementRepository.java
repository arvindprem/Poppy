...
public class GroupManagementRepository {
	...
	public String getAllgroups() {
		...
		try (Session session = sessionFactory.openSession()) {
			...
			Query<Country> countryQuery = session.createQuery("From Country Where CountryId = :COUNTRYID");
			for (PoppyGroup poppyGroup : jsonList) {
				countryQuery.setParameter("COUNTRYID", poppyGroup.getCountry().getId());
				poppyGroup.setCountryLabel(countryQuery.list().get(0).getLabel());
			}
	...
	public boolean editGroupInfo(PoppyGroup groupInfo) {
		...
		try (Session session = sessionFactory.openSession()) {
			Transaction trans = session.beginTransaction();
			Query<Country> countryQuery = session.createQuery("From Country Where CountryLabel = :COUNTRY");
			countryQuery.setParameter("COUNTRY", groupInfo.getCountryLabel());
			PoppyGroup existingGroupInfo = session.get(PoppyGroup.class, groupInfo.getId());
			existingGroupInfo.setLabel(groupInfo.getLabel());
			existingGroupInfo.setNblicence(groupInfo.getNblicence());
			existingGroupInfo.setCountry(countryQuery.list().get(0));
			session.update(existingGroupInfo);
			trans.commit();
			isUpdated = true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean addNewGroup(PoppyGroup poppyGroup) {
		...
		String hql = "FROM Country WHERE CountryLabel = :COUNTRY";

		try (Session session = sessionFactory.openSession()) {
			trans = session.beginTransaction();
			Query<Country> query = session.createQuery(hql);
			query.setParameter("COUNTRY", poppyGroup.getCountryLabel());
			poppyGroup.setCountry(query.list().get(0));
			session.save(poppyGroup);
			trans.commit();
			isAdded = true;
